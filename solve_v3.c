...
void insert(int p_idx, int c_idx, long long cost){
    if (size >= MAX_N){  // 修复：size > MAX_N 改为 size >= MAX_N（避免数组越界）
...
Result solve_v2(int N, long long D, int* prices, int* coupons){
    ...
    // 排序后保留一份，用于后续无券购买（找最小价格）
    int* sorted_prices = (int*)malloc(N * sizeof(int));
    memcpy(sorted_prices, prices, N * sizeof(int));
    qsort(sorted_prices, N, sizeof(int), compareAsc);
    int min_price = sorted_prices[0];  // 无券时买最便宜的商品

    // 优惠券排序（不影响sorted_prices）
    qsort(coupons, N, sizeof(int), compareDesc);

    insert(0, 0, (long long)(sorted_prices[0] - coupons[0]));  // 用排序后的商品价格计算成本
    ...
    while (size > 0){
        ...
        if (next_c_idx < N){
            long long next_cost = (long long)(sorted_prices[current_p_idx] - coupons[next_c_idx]);
            if (next_cost <= D){
        ...
        if (current_c_idx == 0){
            ...
            if (next_p_idx < N){
                long long next_cost = (long long)(sorted_prices[next_p_idx] - coupons[0]);
                if (next_cost <= D){
    ...
    // 新增逻辑：无券购买阶段（用券组合买完后，批量买最便宜的商品）
    if (D >= min_price) {
        int add_count = D / min_price;
        count += add_count;
        D = D % min_price;
    }
